  ...
  useEffect(() => {
    ...
    function drawPlanet() {
      ...
      for (let m = 0; m < 6; m++) {
        ...
        for (let t = -Math.PI / 2; t <= Math.PI / 2; t += 0.05) {
          const x3d = Math.cos(t) * Math.sin(mAngle);
          const z3d = Math.cos(t) * Math.cos(mAngle);
          if (z3d < -0.05) continue;
          const sx = cx + x3d * r;
          const sy = cy + Math.sin(t) * r;
          if (t === -Math.PI / 2 || z3d < 0) {
            ctx!.moveTo(sx, sy);
          } else {
            ctx!.lineTo(sx, sy);
          }
    ...
    function draw() {
      ...
      for (const d of digits) {
        ...
        d.y += d.speed;
        if (d.y > h + 20) {
          Object.assign(d, makeDigit(false));
        }
        // Randomly change digit
        if (Math.random() < 0.005) {
          d.char = String(Math.floor(Math.random() * 10));
        }
      }
          ...
          <div className="space-y-4">
            <div className="bg-white/5 border border-white/10 rounded-xl p-5">
              ...
              <p className="text-white/25 text-xs mt-3">
                Press /start in the bot to complete sign-in
              </p>
            </div>
            <button
              onClick={startAuth}
