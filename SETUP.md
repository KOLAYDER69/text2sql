...
## Быстрый старт (Docker) — для бизнеса

Самый простой способ запустить text2SQL. Нужен только Docker.

### 1. Склонируйте репозиторий

```bash
git clone https://github.com/KOLAYDER69/text2sql.git
cd text2sql
```
...
Откройте http://localhost:3000 — появится мастер настройки:

1. Вставьте URL вашей базы данных (PostgreSQL, MySQL или SQLite)
2. Вставьте Anthropic API ключ (получить на console.anthropic.com)
...
- **Docker** и **Docker Compose** (установлены на компьютере)
- **URL базы данных** — та база, к которой хотите задавать вопросы
- **Anthropic API ключ** — для работы AI (console.anthropic.com, нужно пополнить баланс)

### Стоимость

Один запрос ≈ $0.06. При 100 запросах в день ≈ $6/день.
